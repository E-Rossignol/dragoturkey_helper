    ...
    def _generate(self):
        ...
        try:
            with open(out, "w", encoding="utf-8") as f:
                f.write(f"Toast(Message, Duration := 2000){{\n")
                f.write(f"\tGui, +AlwaysOnTop +ToolWindow -Caption\n")
                f.write(f"\tGui, Color, 000000\n")
                f.write(f"\tGui, Font, s16 cWhite, Arial\n")
                f.write(f"\tGui, Add, Text,, %Message%\n")
                f.write(f"\tx := A_ScreenWidth - 300\n")
                f.write(f"\tGui, Show, x%x% y20 w280 h50 NoActivate\n")
                f.write(f"\tSetTimer, CloseToast, %Duration%\n")
                f.write(f"\treturn\n")
                f.write(f"\n")
                f.write(f"\tCloseToast:\n")
                f.write(f"\tGui, Destroy\n")
                f.write(f"\treturn\n")
                f.write(f"}}\n")
                f.write(f"\n")
                f.write(f"Toast(\"Script lancé\", 2000)\n")
                f.write(f"\n")
                f.write(f"toggle := false\n")
                f.write(f"\n")
                f.write(f"{t}::\n")
                f.write(f"\ttoggle := !toggle\n")
                f.write(f"\tif (toggle) {{\n")
                f.write(f"\t\tToast(\"Macro activée\", 2000)\n")
                f.write(f"\t\tSetTimer, MyLoop, 0\n")
                f.write(f"\t}} else {{\n")
                f.write(f"\t\tToast(\"Macro désactivée\", 2000)\n")
                f.write(f"\t\tSetTimer, MyLoop, Off\n")
                f.write(f"\t}}\n")
                f.write(f"return\n")
                f.write(f"\n")
                f.write(f"^F11::\n")
                f.write(f"\tToast(\"Script arrêté\", 2000)\n")
                f.write(f"\tSleep, 2000\n")
                f.write(f"\tExitApp\n")
                f.write(f"return\n")
                f.write(f"\n")
                f.write(f"MyLoop:\n")
                f.write(f"\tSend, {r}\n")
                f.write(f"\tSleep, 3500\n")
                f.write(f"\tSend, {a}\n")
                f.write(f"\tSleep, 3500\n")
                f.write(f"return\n")
                f.write(f"\n")
